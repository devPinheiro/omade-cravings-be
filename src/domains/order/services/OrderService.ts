...
export class OrderService {
  ...
  async createOrder(data: CreateOrderData): Promise<Order> {
    ...
    try {
      ...
      // Update inventory
      await this.updateInventory(cartItems, transaction);
  ...
  async updateOrder(id: string, data: UpdateOrderData): Promise<Order | null> {
    ...
    if (data.status && previousStatus !== data.status && updatedOrder) {
      try {
  ...
  private async updateInventory(cartItems: any[], transaction: Transaction): Promise<void> {
    for (const item of cartItems) {
